...
GAME.BulletManager.prototype.update = function() {
	...
	for (var a = this.bullets, b = 0; b < a.length; b++) {
		var c = a[b];
		c.position.y -= 20 * GAME.time.DELTA_TIME; - 50 > c.position.y && (a.splice(b, 1), b--, this.bulletPool.returnObject(c), this.engine.view.actionContainer.removeChild(c))
	}
};
...
GAME.LaserManager = function(a) {
	...
	this.bullets = [];
	this.fireCount = 0;
	this.view = new GAME.LaserView(PIXI.Texture.fromImage("img/MEGAlaser.png"));
...
GAME.LaserManager.prototype.update = function() {
	...
	for (var a = this.bullets, b = 0; b < a.length; b++) {
		var c = a[b];
		c.position.y -= 10; - 150 > c.position.y && (a.splice(b, 1), b--, this.bulletPool.returnObject(c))
	}
};
...
GAME.EnemyManager.prototype.update = function() {
	...
	var a = (Math.sin(this.positionShift) + 1) / 2,
		b = this.spread / 2,
...
GAME.EnemyManager.prototype.changeWave = function() {
	...
	var a = 300 - this.halfSpread,
		b = 300 + this.halfSpread;
...
GAME.EnemyBulletManager.prototype.fire = function(a, b) {
	...
	var e = c.position.x - this.engine.player.position.x,
		d = c.position.y - this.engine.player.position.y,
		h = Math.sqrt(e * e + d * d);
	c.speedX = e / h * b;
	c.speedY = d / h * b
};
...
GAME.PickupManager.prototype.dropPickup = function() {
	if (this.engine.canSpawn) {
		var a = this.pickupPool.getObject(),
			b;
...
GAME.PickupManager.prototype.destroyPickup = function(a) {
	for (var b = this.pickups.length, c = 0; c < b; c++)
		if (this.pickups[c] == a) {
...
GAME = GAME || {};
GAME.EMP = function() {
	PIXI.Sprite.call(this, PIXI.Texture.fromImage("img/EMP.png"));
	this.blendMode = PIXI.blendModes.SCREEN;
	this.firstPhase = !0;
	this.isDead = !1;
	this.anchor.x = 0.5;
...
GAME.RocketManager.prototype.update = function() {
	...
	for (var a = this.rockets, b = a.length, c = 0; c < b; c++) {
		var e = a[c];
		e.update(); - 50 > e.position.y && (a.splice(c, 1), c--, b--, this.rocketPool.returnObject(e), this.engine.view.actionContainer.removeChild(e));
...
GAME.RocketManager.prototype.destroyRocket = function(a) {
	for (var b = this.rockets.length, c = 0; c < b; c++)
		if (this.rockets[c] == a) {
...
GAME.CollisionManager.prototype.hitTestLaserVsEnemy = function() {
	for (var a = this.engine.bulletManager.bullets, b = this.engine.enemyManager.enemies, c = a.length, e = 0; e < c; e++)
		for (var d = a[e], h = b.length, f = 0; f < h; f++) {
			var g = b[f],
				j = d.position.x - g.position.x,
				m = d.position.y - g.position.y,
				k = d.radius + g.radius;
			if (j * j + m * m < k * k) {
...
GAME.CollisionManager.prototype.hitTestSuperLaserVsEnemy = function() {
	for (var a = this.engine.laserManager.bullets, b = this.engine.enemyManager.enemies, c = a.length, e = 0; e < c; e++)
		for (var d = a[e], h = b.length, f = 0; f < h; f++) {
			var g = b[f],
				j = d.position.x - g.position.x,
				m = d.position.y - g.position.y,
...
GAME.CollisionManager.prototype.hitTestRocketsVsEnemy = function() {
	for (var a = this.engine.rocketManager.rockets, b = this.engine.enemyManager.enemies, c = a.length, e = 0; e < c; e++)
		for (var d = a[e], h = b.length, f = 0; f < h; f++) {
			var g = b[f],
				j = d.position.x - g.position.x,
				m = d.position.y - g.position.y,
...
GAME.CollisionManager.prototype.hitTestPlayerVsPickups = function() {
	for (var a = this.engine.player, b = this.engine.pickupManager.pickups, c = b.length, e = 0; e < c; e++) {
		var d = b[e],
			h = a.position.x - d.position.x,
			f = a.position.y - d.position.y,
			g = a.radius + d.radius;
		if (h * h + f * f < g * g) {
...
GAME.CollisionManager.prototype.hitTestPlayerVsEnemy = function() {
	for (var a = this.engine.player, b = this.engine.enemyManager.enemies, c = b.length, e = 0; e < c; e++) {
		var d = b[e],
			h = a.position.x - d.position.x,
			f = a.position.y - 28 - 1 - d.position.y,
			g = a.radius + d.radius;
		if (h * h + f * f < g * g) {
...
GAME.CollisionManager.prototype.hitTestPlayerVsEnemyBullet = function() {
	for (var a = this.engine.player, b = this.engine.enemyBulletManager.bullets, c = b.length, e = 0; e < c; e++) {
		var d = b[e],
			h = a.position.x - d.position.x,
			f = a.position.y - 19 - d.position.y,
			g = a.radius + d.radius;
		if (h * h + f * f < g * g) {
...
GAME.ShootyView = function(a) {
	...
	this.vignette = PIXI.Sprite.fromImage("img/vignette.png");
...
GAME.addHitArea = function(a) {
	var b = PIXI.Sprite.fromImage("img/hitTest.png");
	b.anchor.x = 0.5;
	b.anchor.y = 0.5;
